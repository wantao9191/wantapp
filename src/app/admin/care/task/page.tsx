...
export default function OrganizationsPage() {
  ...
  // 操作按钮
  const careTaskActions: ActionConfig = {
    title: '操作',
    key: 'actions',
    width: 150,
    fixed: 'left',
  ...
  const getList = (params: Record<string, any>) => {
    return http.get('/admin/careTasks', params)
  }
  const handleAdd = () => {
    setFormData(null)
    setOpen(true)
  ...
  const handleDelete = (record: any) => {
    confirm({
      title: '删除护理任务后，该护理任务将无法正常使用，确定删除该护理任务吗？',
      icon: <ExclamationCircleFilled />,
      okText: '确定',
      cancelText: '取消',
      onOk: () => {
        return new Promise((resolve, reject) => {
          http.delete(`/admin/careTasks/${record.id}`).then(() => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={null}
        open={open}
        title={formData?.id ? '编辑护理任务' : '新增护理任务'}
