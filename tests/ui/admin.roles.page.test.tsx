...
import RolesPage from '@/app/admin/system/roles/page'

// Mock 所有外部依赖
vi.mock('@/app/admin/system/roles/useItems', () => ({
  default: vi.fn()
}))
...
vi.mock('@/app/admin/system/roles/components/editModal', () => ({
  default: vi.fn(({ formData, onSubmit, onCancel }) => (
    <div data-testid="edit-modal">
      <div data-testid="modal-data">
        {formData ? JSON.stringify(formData) : '新增角色'}
      </div>
...
describe('RolesPage', () => {
  ...
  beforeEach(() => {
    ...
    const useItems = require('@/app/admin/system/roles/useItems').default
  ...
  it('应该处理新增角色操作', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('modal-data')).toHaveTextContent('新增角色')
    })
  })
  ...
  it('应该正确处理表单数据传递', async () => {
    ...
    await waitFor(() => {
      ...
      expect(modalData).toHaveTextContent('新增角色')
    })
  })
