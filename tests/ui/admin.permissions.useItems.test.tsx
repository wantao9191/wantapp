...
import useItems from '@/app/admin/system/permissions/useItems'
...
describe('useItems - Permissions', () => {
  ...
  it('应该返回正确的表格列配置', () => {
    ...
    expect(columnNames).toContain('权限名称')
    expect(columnNames).toContain('权限编码')
    expect(columnNames).toContain('菜单编码')
  ...
  it('应该正确配置权限名称搜索字段', () => {
    ...
    expect(nameField?.label).toBe('权限名称')
  ...
  it('应该正确配置表格列属性', () => {
    ...
    expect(nameColumn?.title).toBe('权限名称')
    ...
    expect(codeColumn?.title).toBe('权限编码')
    expect(codeColumn?.key).toBe('code')
    expect(codeColumn?.width).toBe(200)
    
    // 验证菜单编码列
    const menuIdColumn = result.current.tableColumns.find(col => col.dataIndex === 'menuId')
    expect(menuIdColumn).toBeDefined()
    expect(menuIdColumn?.title).toBe('菜单编码')
    expect(menuIdColumn?.key).toBe('menuId')
    expect(menuIdColumn?.width).toBe(130)
    ...
    expect(descColumn?.width).toBe(100)
  ...
  it('应该正确配置状态列的渲染函数', () => {
    ...
    // 测试渲染函数返回的组件结构
    const result1 = renderFn(1, { id: 1, name: '测试权限' }) // 启用状态
    const result2 = renderFn(0, { id: 2, name: '测试权限' }) // 禁用状态
  ...
  it('应该正确处理状态切换', async () => {
    ...
    // 模拟状态切换
    const record = { id: 1, name: '测试权限', status: 1 }
